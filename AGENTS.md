...
## Project Overview
- `SideHustle` is a small .NET 8 WinForms tray app.
- Entry point:
...
- Current behavior:
  ...
  - The center window is allowed to overlap the unusable portion of Fan Control's right side and uses a small overlap on the Steam Friends edge to hide seams.
...
- For a single-file release build, use:
  - `dotnet publish .\SideHustle.csproj -c Release -r win-x64 -p:PublishSingleFile=true -p:SelfContained=true -o .\publish`
...
- If VS Code debugging is involved:
  - Use `.vscode/launch.json` to launch `bin/Debug/net8.0-windows/SideHustle.exe` directly.
...
- `SideHustle.csproj` only when changing project settings or framework behavior.
